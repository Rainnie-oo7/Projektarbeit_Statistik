...
def plot_combined_balken(df_prim, df_sec, prim_startg, prim_endg, prim_startp, prim_endp, sec_startg, sec_endg, sec_startp, sec_endp, title):
    ...
    for i, row in df_plot.iterrows():
        ...
        # Primärbalken #Gesetzlich und Text
        if pd.notna(start1) and pd.notna(end1):
            ax.barh(i+0.2, (end1 - start1).days, left=start1, height=0.4,
                    color='bisque', edgecolor='black')
            # Falls es ein Vorgespräch gibt → "I" in den primären Balken zeichnen
            if "Vorgespräch" in row and pd.notna(row["Vorgespräch"]):
                vorgespraech_datum = pd.to_datetime(row["Vorgespräch"], format='%d.%m.%y', errors='coerce')
                if pd.notna(vorgespraech_datum) and start1 <= vorgespraech_datum <= end1:
        ...
        if not match.empty:
            ...
            if pd.notna(sec_start_dateg) and pd.notna(sec_end_dateg):
                ...
                if "Vorgespräch" in sec_row and pd.notna(sec_row["Vorgespräch"]):
                    vorgespraech_datum_sec = pd.to_datetime(sec_row["Vorgespräch"], format='%d.%m.%y', errors='coerce')
                    if pd.notna(vorgespraech_datum_sec) and sec_start_dateg <= vorgespraech_datum_sec <= sec_end_dateg:
                        ax.text(vorgespraech_datum_sec, i + 0.25, 'I', va='center', ha='center',
                                fontsize=16, fontweight='bold', color='green', alpha=0.3)
            if pd.notna(sec_start_dateg) and pd.isna(sec_end_dateg):
            ...
            if pd.notna(sec_start_datep) and pd.isna(sec_end_datep):
    ...
    ax.invert_yaxis()
    ax.set_xlim(datetime(2025, 4, 1), datetime(2026, 4, 9))
